...
client.on('messageCreate', msg => {
  // Skip our own messages. Gate() in handleInbound() handles all other filtering
  // (DM pairing, channel allowlist, requireMention + mentionPatterns) so other
  // bots that mention us are processed identically to human senders.
...
async function handleInbound(msg: Message): Promise<void> {
  ...
  // Channel bot discovery — fetch last 20 messages, collect unique bot authors
  // (excluding self) so the LLM can auto-mention cross-bot collaborators.
  // Failure-tolerant: if the fetch fails, just omit channel_bots.
  ...
  mcp.notification({
    ...
    params: {
      ...
      meta: {
        ...
        ...(channelBots.length > 0 ? { channel_bots: channelBots.join(',') } : {}),
      },
    },
  }).catch(err => {
